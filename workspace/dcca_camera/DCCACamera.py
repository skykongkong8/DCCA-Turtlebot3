...
class DCCACamera(RGBDRealsenseCamera):
    def __init__(self):
        ...
        self.device = model_setting[2]
        self.names = model_setting[3]
        print("DCCACamera Initialized...")

    def yolov5(self, cvView = False):
        flag, rgb_img, depth_img = self.get_frame_stream()
        if flag:
            detected_image = self.yoloManager.process_model_in_loop(rgb_img, self.model, self.stride, self.device, self.names)
